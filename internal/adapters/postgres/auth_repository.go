...
func (s *Store) CreateUserWithPassword(ctx context.Context, email, displayName, passwordHash string) (auth.User, error) {
	var user auth.User
	var ledgerID string
...
func (s *Store) CreateUserWithIdentity(ctx context.Context, params auth.CreateIdentityParams) (auth.User, auth.AuthIdentity, error) {
	var user auth.User
...
func (s *Store) CreateAuthIdentity(ctx context.Context, params auth.CreateIdentityParams) (auth.AuthIdentity, error) {
	var identity auth.AuthIdentity
...
func (s *Store) UpdateAuthIdentityLogin(ctx context.Context, userID, provider string) error {
...
func (s *Store) GetUserByID(ctx context.Context, userID string) (auth.User, error) {
	var user auth.User
	row := s.pool.QueryRow(ctx, `
		SELECT id, email, COALESCE(display_name, ''), COALESCE(avatar_url, ''), email_verified_at, is_active, created_at, updated_at
		FROM users
...
func (s *Store) GetAuthSecretHash(ctx context.Context, userID string) (string, error) {
	var hash string
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) CreateAuthSession(ctx context.Context, params auth.CreateSessionParams) (auth.AuthSession, error) {
...
func (s *Store) RotateAuthSession(ctx context.Context, params auth.RotateSessionParams) (auth.AuthSession, auth.User, error) {
	var session auth.AuthSession
	var user auth.User
...
func (s *Store) RevokeAuthSession(ctx context.Context, refreshTokenHash string) error {
...
func (s *Store) CreateOAuthState(ctx context.Context, provider, state, codeVerifier, redirectURI string) (auth.OAuthState, error) {
...
func (s *Store) GetOAuthState(ctx context.Context, state string) (auth.OAuthState, error) {
	var saved auth.OAuthState
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) MarkOAuthStateUsed(ctx context.Context, stateID string) error {
	_, err := s.pool.Exec(ctx, `
		UPDATE oauth_states
...
func (s *Store) GetAuthIdentityByProvider(ctx context.Context, provider, providerUserID string) (auth.AuthIdentity, error) {
	var identity auth.AuthIdentity
	row := s.pool.QueryRow(ctx, `
