...
func (s *Store) GetPlanByLedger(ctx context.Context, ledgerID string) (budget.Plan, error) {
...
func (s *Store) CreatePlan(ctx context.Context, ledgerID, name string) (budget.Plan, error) {
	var plan budget.Plan
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) UpdatePlan(ctx context.Context, ledgerID, name string, updatedAt time.Time) (budget.Plan, error) {
	var plan budget.Plan
	row := s.pool.QueryRow(ctx, `
		UPDATE budget_plans
...
func (s *Store) ListVersions(ctx context.Context, ledgerID string, from, to *time.Time) ([]budget.Version, error) {
	query := `
...
func (s *Store) GetVersion(ctx context.Context, ledgerID, versionID string) (budget.Version, error) {
	var version budget.Version
	row := s.pool.QueryRow(ctx, `
		SELECT v.id, v.plan_id, v.effective_from_month, v.created_by_user_id, v.created_at, v.updated_at
		FROM budget_plan_versions v
...
func (s *Store) CreateVersionWithLines(ctx context.Context, ledgerID, planID, userID string, effectiveFrom time.Time, lines []budget.LineInput) (budget.Version, error) {
	var version budget.Version
...
func (s *Store) AddLine(ctx context.Context, versionID string, line budget.LineInput) (budget.Line, error) {
...
func (s *Store) UpdateLine(ctx context.Context, lineID string, percent float64, includeChildren bool, updatedAt time.Time) (budget.Line, error) {
...
func (s *Store) DeleteLine(ctx context.Context, lineID string) error {
	cmd, err := s.pool.Exec(ctx, `DELETE FROM budget_plan_lines WHERE id = $1`, lineID)
	if err != nil {
		return err
	}
...
func (s *Store) GetLinesByVersion(ctx context.Context, versionID string) ([]budget.Line, error) {
...
func (s *Store) GetApplicableVersion(ctx context.Context, ledgerID string, month time.Time) (budget.Version, error) {
	var version budget.Version
	row := s.pool.QueryRow(ctx, `
		SELECT v.id, v.plan_id, v.effective_from_month, v.created_by_user_id, v.created_at, v.updated_at
		FROM budget_plan_versions v
...
func (s *Store) GetCategoryInfo(ctx context.Context, ledgerID string, categoryIDs []string) (map[string]budget.CategoryInfo, error) {
	rows, err := s.pool.Query(ctx, `
...
func (s *Store) GetCategoryDescendants(ctx context.Context, ledgerID, categoryID string) ([]string, error) {
	rows, err := s.pool.Query(ctx, `
...
func (s *Store) IncomeBaseForMonth(ctx context.Context, ledgerID string, month time.Time) (int64, error) {
	start := month
...
func (s *Store) SpentActualForMonth(ctx context.Context, ledgerID string, categoryIDs []string, month time.Time) (int64, error) {
	start := month
	end := month.AddDate(0, 1, 0)
	var total int64
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) OutsideBudgetForMonth(ctx context.Context, ledgerID string, month time.Time) (int64, error) {
	start := month
	end := month.AddDate(0, 1, 0)
	var total int64
	row := s.pool.QueryRow(ctx, `
