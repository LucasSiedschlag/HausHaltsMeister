...
func (s *Store) ListCardNetworks(ctx context.Context) ([]creditcard.CardNetwork, error) {
...
func (s *Store) CreateCardNetwork(ctx context.Context, code, displayName string) (creditcard.CardNetwork, error) {
	var item creditcard.CardNetwork
...
func (s *Store) UpdateCardNetwork(ctx context.Context, code, displayName string, updatedAt time.Time) (creditcard.CardNetwork, error) {
	var item creditcard.CardNetwork
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) DeleteCardNetwork(ctx context.Context, code string) error {
	cmd, err := s.pool.Exec(ctx, `DELETE FROM card_networks WHERE code = $1`, code)
	if err != nil {
		return err
	}
...
func (s *Store) GetCardNetwork(ctx context.Context, code string) (creditcard.CardNetwork, error) {
	var item creditcard.CardNetwork
	row := s.pool.QueryRow(ctx, `
		SELECT code, display_name, created_at, updated_at
		FROM card_networks
...
func (s *Store) ListCreditCards(ctx context.Context, ledgerID string) ([]creditcard.CreditCard, error) {
	rows, err := s.pool.Query(ctx, `
...
func (s *Store) GetCreditCard(ctx context.Context, ledgerID, cardAccountID string) (creditcard.CreditCard, error) {
	var card creditcard.CreditCard
	row := s.pool.QueryRow(ctx, `
		SELECT c.account_id, a.ledger_id, c.issuer_name, c.network, c.nickname, c.last4, c.credit_limit_cents, c.closing_day, c.due_day, c.created_at, c.updated_at
		FROM credit_cards c
...
func (s *Store) CreateCreditCard(ctx context.Context, params creditcard.CreditCard) (creditcard.CreditCard, error) {
	var card creditcard.CreditCard
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) UpdateCreditCard(ctx context.Context, params creditcard.CreditCard, updatedAt time.Time) (creditcard.CreditCard, error) {
	var card creditcard.CreditCard
	row := s.pool.QueryRow(ctx, `
		UPDATE credit_cards
...
func (s *Store) DeleteCreditCard(ctx context.Context, ledgerID, cardAccountID string) error {
	cmd, err := s.pool.Exec(ctx, `DELETE FROM credit_cards WHERE account_id = $1`, cardAccountID)
	if err != nil {
		return err
	}
	if cmd.RowsAffected() == 0 {
...
func (s *Store) GetAccountType(ctx context.Context, ledgerID, accountID string) (string, error) {
...
func (s *Store) GetAccountLedger(ctx context.Context, accountID string) (string, error) {
	var ledgerID string
	row := s.pool.QueryRow(ctx, `SELECT ledger_id FROM accounts WHERE id = $1`, accountID)
...
func (s *Store) GetCategoryBudgetInfo(ctx context.Context, ledgerID, categoryID string) (string, bool, error) {
...
func (s *Store) CreatePlanWithInstallments(ctx context.Context, plan creditcard.InstallmentPlan, installments []creditcard.Installment) (creditcard.InstallmentPlan, error) {
...
func (s *Store) ListPlans(ctx context.Context, ledgerID, cardAccountID string, status *string) ([]creditcard.InstallmentPlan, error) {
	query := `
...
func (s *Store) GetPlan(ctx context.Context, ledgerID, cardAccountID, planID string) (creditcard.InstallmentPlan, error) {
	var plan creditcard.InstallmentPlan
	row := s.pool.QueryRow(ctx, `
		SELECT id, ledger_id, card_account_id, purchase_occurred_at, merchant, description, category_id, total_amount_cents, installments_count, installment_amount_cents, first_due_month, status, created_by_user_id, created_at, updated_at
		FROM installment_plans
...
func (s *Store) UpdatePlanStatus(ctx context.Context, ledgerID, planID, status string, updatedAt time.Time) error {
...
func (s *Store) ListInstallments(ctx context.Context, ledgerID, cardAccountID string, month *time.Time, status *string) ([]creditcard.Installment, error) {
	query := `
...
func (s *Store) UpdateInstallmentStatus(ctx context.Context, ledgerID, installmentID, status string, updatedAt time.Time) (creditcard.Installment, error) {
	var item creditcard.Installment
	row := s.pool.QueryRow(ctx, `
		UPDATE installments
		SET status = $2, updated_at = $3
...
func (s *Store) ListInstallmentsForPosting(ctx context.Context, ledgerID, cardAccountID string, month time.Time) ([]creditcard.Installment, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT i.id, i.ledger_id, i.plan_id, i.installment_no, i.due_month, i.amount_cents, i.status, i.posted_transaction_id, i.paid_statement_id, i.created_at, i.updated_at
		FROM installments i
		JOIN installment_plans p ON p.id = i.plan_id
...
func (s *Store) GetPlanCategory(ctx context.Context, ledgerID, planID string) (string, error) {
...
func (s *Store) MarkInstallmentPosted(ctx context.Context, ledgerID, installmentID, transactionID string, updatedAt time.Time) error {
	cmd, err := s.pool.Exec(ctx, `
		UPDATE installments
...
func (s *Store) GetStatement(ctx context.Context, ledgerID, cardAccountID, statementID string) (creditcard.Statement, error) {
...
func (s *Store) GetStatementByMonth(ctx context.Context, ledgerID, cardAccountID string, month time.Time) (creditcard.Statement, error) {
	var statement creditcard.Statement
	row := s.pool.QueryRow(ctx, `
		SELECT id, ledger_id, card_account_id, statement_month, closing_date, due_date, total_charges_cents, total_payments_cents, status, payment_transaction_id, created_at, updated_at
		FROM credit_card_statements
...
func (s *Store) CreateStatement(ctx context.Context, statement creditcard.Statement) (creditcard.Statement, error) {
...
func (s *Store) UpdateStatementTotals(ctx context.Context, statementID string, totalCharges, totalPayments int64, status string, updatedAt time.Time) (creditcard.Statement, error) {
...
func (s *Store) ListStatements(ctx context.Context, ledgerID, cardAccountID string, month *time.Time) ([]creditcard.Statement, error) {
	query := `
		SELECT id, ledger_id, card_account_id, statement_month, closing_date, due_date, total_charges_cents, total_payments_cents, status, payment_transaction_id, created_at, updated_at
		FROM credit_card_statements
		WHERE ledger_id = $1 AND card_account_id = $2
...
func (s *Store) SetStatementPayment(ctx context.Context, statementID, paymentTransactionID string, totalPayments int64, status string, updatedAt time.Time) (creditcard.Statement, error) {
	var updated creditcard.Statement
	row := s.pool.QueryRow(ctx, `
		UPDATE credit_card_statements
...
func (s *Store) MarkInstallmentsPaid(ctx context.Context, ledgerID, cardAccountID string, month time.Time, statementID string, updatedAt time.Time) error {
...
func (s *Store) SumStatementCharges(ctx context.Context, ledgerID, cardAccountID string, month time.Time) (int64, error) {
	var total int64
	row := s.pool.QueryRow(ctx, `
...
func (s *Store) HasPostedInstallments(ctx context.Context, ledgerID, planID string) (bool, error) {
	var exists bool
	row := s.pool.QueryRow(ctx, `
		SELECT EXISTS (
			SELECT 1
