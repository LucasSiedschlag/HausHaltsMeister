...
func (s *Store) ListAccountBalances(ctx context.Context, ledgerID string, cutoff time.Time) ([]reports.AccountBalance, error) {
...
func (s *Store) ListCategorySummary(ctx context.Context, ledgerID string, from, to time.Time) ([]reports.CategorySummary, error) {
	rows, err := s.pool.Query(ctx, `
...
func (s *Store) ListCashflow(ctx context.Context, ledgerID string, from, to time.Time) ([]reports.CashflowEntry, error) {
	rows, err := s.pool.Query(ctx, `
