...
func TestUC14_LendMoney(t *testing.T) {
	// Setup DB
	...
	_, _ = catRepo.Create(ctx, &category.Category{Name: "Picuinhas", Direction: "OUT", IsActive: true})

	// Create Person
	person, _ := picService.CreatePerson(ctx, "Bob", "")

	t.Run("Success - Lend Money with Auto Flow", func(t *testing.T) {
		...
		assert.Equal(t, 100.0, entryRes["amount"])
		assert.Equal(t, "PLUS", entryRes["kind"])
