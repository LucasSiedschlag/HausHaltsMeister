...
func TestUC10_BudgetManagement(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 500.0, itemRes["PlannedAmount"])
	...
	assert.Equal(t, "Food", item["CategoryName"]) // Assuming API returns enriched name?
	// If API doesn't enrich name, just ID.
	// Domain model BudgetItem usually has CategoryID.
	// Service GetBudgetSummary doesn't explicitly loop to fetch Category Names to enrich?
	// `period.Items` are from `repo.GetItemsByPeriod`.
	// Usually repo joins to get Name. I'll verify if test fails.

	assert.Equal(t, 500.0, item["PlannedAmount"])
	assert.Equal(t, 150.0, item["ActualAmount"]) // From CashFlow aggregation
}
