...
const pageSubtitle = computed(() => 'Registre entradas manuais e acompanhe o total mensal.')
...
const categoryOptions = computed(() =>
  categories.value.filter((category) => category.direction === 'IN' && category.is_active),
)
...
async function fetchEntries() {
  ...
  try {
    entries.value = await listCashflows(monthParam.value, { direction: 'IN' })
  } catch (error) {
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div class="space-y-1">
        <h1 class="text-2xl font-semibold tracking-tight">Entradas</h1>
        <p class="text-sm text-muted-foreground">{{ pageSubtitle }}</p>
      </div>
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
        <div class="space-y-1">
          <Label for="month-entradas">Mês de referência</Label>
          <MonthPicker id="month-entradas" v-model="monthValue" :disabled="loading" />
        </div>
    ...
    <CashflowTable
      ...
      description="Lançamentos de entrada registrados no período selecionado."
      @create="openCreate"
      @edit="openEdit"
